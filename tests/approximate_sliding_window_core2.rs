...
fn test_weighted_contribution_calculation() {
    ...
    assert_eq!(counter.try_acquire_at(30, 5), Ok(()));   // Window 0 [0-9]
    assert_eq!(counter.try_acquire_at(40, 15), Ok(()));  // Window 1 [10-19]
    ...
    assert_eq!(counter.try_acquire_at(20, 18), Ok(()));
    ...
    assert_eq!(counter.try_acquire_at(50, 18), Err(RateLimitError::ExceedsCapacity));
}
...
fn test_no_overlap_scenario() {
    ...
    assert_eq!(counter.try_acquire_at(50, 25), Ok(()));
}
...
fn test_full_overlap_scenario() {
    ...
    // Window 0: fill
    assert_eq!(counter.try_acquire_at(40, 10), Ok(()));  // Window 0 [0-14]
...
fn test_window_switching_consistency() {
    ...
    // Add to Window 0
    assert_eq!(counter.try_acquire_at(25, 8), Ok(()));   // Window 0 [0-11]
    
    // Switch to Window 1
    assert_eq!(counter.try_acquire_at(30, 15), Ok(()));  // Window 1 [12-23]
    
    // Switch back to Window 0 (new cycle)
    assert_eq!(counter.try_acquire_at(20, 28), Ok(()));  // Window 0 [24-35]
...
fn test_same_tick_multiple_calls() {
    ...
    assert_eq!(counter.try_acquire_at(1, 10), Err(RateLimitError::ExceedsCapacity));
...
fn test_window_starts_alignment() {
    ...
    // Window 0 [0-7]
    assert_eq!(counter.try_acquire_at(30, 3), Ok(()));
    
    // Window 1 [8-15]
    assert_eq!(counter.try_acquire_at(40, 12), Ok(()));
    
    // Window 0 new cycle [16-23]
    assert_eq!(counter.try_acquire_at(35, 20), Ok(()));
    ...
    assert_eq!(counter.try_acquire_at(50, 24), Ok(()));
    
    // Verify remaining capacity: 960 - 645 = 315
    // New request: 40 * 8 = 320 > 315, should fail
    assert_eq!(counter.try_acquire_at(40, 24), Err(RateLimitError::ExceedsCapacity));
    
    // But 39 * 8 = 312 <= 315, should succeed
    assert_eq!(counter.try_acquire_at(39, 24), Ok(()));
}
...
fn test_single_tick_window() {
    ...
    assert_eq!(counter.try_acquire_at(1, 1), Err(RateLimitError::ExceedsCapacity));
...
fn test_large_window_size() {
    ...
    assert_eq!(counter.try_acquire_at(200, 500), Ok(()));   // Window 0 [0-999]
    assert_eq!(counter.try_acquire_at(150, 800), Ok(()));   // Still in Window 0
    assert_eq!(counter.try_acquire_at(150, 999), Ok(()));   // Still in Window 0
    
    // Total now 500, at capacity
    assert_eq!(counter.try_acquire_at(1, 999), Err(RateLimitError::ExceedsCapacity));
    
    // Switch to Window 1 [1000-1999]
    assert_eq!(counter.try_acquire_at(101, 1200), Err(RateLimitError::ExceedsCapacity));
    assert_eq!(counter.try_acquire_at(100, 1200), Ok(()));
}
...
fn test_partial_overlap_precision() {
    ...
    assert_eq!(counter.try_acquire_at(40, 25), Err(RateLimitError::ExceedsCapacity));
}
...
fn test_edge_case_overlap_calculation() {
    ...
    assert_eq!(counter.try_acquire_at(40, 19), Ok(()));
