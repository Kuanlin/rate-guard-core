...
fn test_high_frequency_leak() {
    ...
    for i in 1..=10 {
        ...
        assert_eq!(bucket.try_acquire_at(i, 1), Ok(()));
    }

    // At tick 10: remaining = 100 - 10 + 10 = 100, bucket is full
    assert_eq!(bucket.try_acquire_at(10, 90), Err(SimpleRateLimitError::InsufficientCapacity));
}
...
fn test_leak_boundary_timing() {
    ...
    assert_eq!(bucket.try_acquire_at(9, 50), Ok(()));
...
fn test_last_leak_tick_alignment() {
    ...
    assert_eq!(bucket.try_acquire_at(3, 30), Ok(()));
    
    // tick 17: elapsed = 17 - 0 = 17, leak_times = 1, leaked = 5
    // last_leak_tick updates to 0 + 1*10 = 10
    assert_eq!(bucket.try_acquire_at(17, 20), Ok(())); // remaining = 30 - 5 + 20 = 45
    
    // tick 19: elapsed = 19 - 10 = 9, no leak yet (9 < 10)
    assert_eq!(bucket.try_acquire_at(19, 50), Ok(())); // remaining = 45 + 50 = 95
...
fn test_same_tick_multiple_calls() {
    ...
    assert_eq!(bucket.try_acquire_at(5, 30), Ok(()));
    assert_eq!(bucket.try_acquire_at(5, 40), Ok(()));
...
fn test_leak_calculation_precision() {
    ...
    assert_eq!(bucket.try_acquire_at(28, 13), Ok(()));
}