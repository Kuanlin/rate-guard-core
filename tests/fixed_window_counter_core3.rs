...
fn test_rate_limiter_core_window_basics() {
    ...
    assert_eq!(limiter.try_acquire_at(70, 9), Ok(()));
    assert_eq!(limiter.capacity_remaining(9), 0);
    
    // Window is full
    assert_eq!(limiter.try_acquire_at(1, 9), Err(RateLimitError::ExceedsCapacity));
}
...
fn test_rate_limiter_core_window_reset() {
    ...
    assert_eq!(limiter.try_acquire_at(30, 12), Ok(()));
...
fn test_rate_limiter_core_window_boundaries() {
    ...
    // Window 0: [0-4]
    assert_eq!(limiter.try_acquire_at(60, 4), Ok(())); // Last tick of window
    ...
    assert_eq!(limiter.try_acquire_at(100, 5), Ok(()));
...
fn test_rate_limiter_core_time_consistency() {
    ...
    assert_eq!(limiter.try_acquire_at(20, 15), Ok(()));
    
    // Going back before window start should fail
    let result: AcquireResult = limiter.try_acquire_at(10, 9);
...
fn test_rate_limiter_core_burst_at_boundaries() {
    ...
    // End of window 0 [0-9]
    assert_eq!(limiter.try_acquire_at(100, 9), Ok(()));
    assert_eq!(limiter.capacity_remaining(9), 0);
    
    // Immediate burst at window 1 [10-19]
    assert_eq!(limiter.try_acquire_at(100, 10), Ok(()));
    assert_eq!(limiter.capacity_remaining(10), 0);
    
    // Can burst again at next window
    assert_eq!(limiter.try_acquire_at(100, 20), Ok(()));
}
...
fn test_rate_limiter_core_concurrent_window_access() {
    ...
    for i in 0..10 {
        ...
        let handle = thread::spawn(move || {
            ...
            limiter_clone.try_acquire_at(15, i % 10)
        });
        handles.push(handle);
    }
...
fn test_rate_limiter_core_interface_consistency() {
    ...
    assert_eq!(limiter.try_acquire_at(25, 7), Ok(()));
    assert_eq!(limiter.capacity_remaining(10), 50);
    
    // Use exact remaining
    assert_eq!(limiter.try_acquire_at(50, 12), Ok(()));
...
fn test_rate_limiter_core_as_trait_object() {
    ...
    // Window 0 [0-4]
    assert_eq!(limiter.try_acquire_at(20, 2), Ok(()));
    assert_eq!(limiter.try_acquire_at(20, 4), Ok(()));
    assert_eq!(limiter.capacity_remaining(4), 10);
    
    // Window 1 [5-9]
    assert_eq!(limiter.try_acquire_at(50, 6), Ok(()));
    assert_eq!(limiter.capacity_remaining(8), 0);
    
    // Window 2 [10-14]
    assert_eq!(limiter.try_acquire_at(25, 11), Ok(()));
    assert_eq!(limiter.try_acquire_at(25, 13), Ok(()));
    assert_eq!(limiter.capacity_remaining(14), 0);
...
fn test_rate_limiter_core_window_alignment() {
    ...
    assert_eq!(limiter.try_acquire_at(50, 13), Ok(()));
    ...
    assert_eq!(limiter.try_acquire_at(30, 28), Ok(()));
