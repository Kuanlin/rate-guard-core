...
/// Helper function to create a FixedWindowCounterCore as RateLimitCore
fn create_fixed_window_limiter(capacity: Uint, window_ticks: Uint) -> Box<dyn RateLimitCore> {
    Box::new(FixedWindowCounterCore::new(capacity, window_ticks))
}

#[test]
fn test_rate_limiter_core_window_basics() {
    let limiter: Box<dyn RateLimitCore> = create_fixed_window_limiter(100, 10);
    ...
    assert_eq!(limiter.capacity_remaining_or_0(9), 0);
...
fn test_rate_limiter_core_window_reset() {
    let limiter: Box<dyn RateLimitCore> = create_fixed_window_limiter(50, 10);
    ...
    assert_eq!(limiter.capacity_remaining_or_0(10), 50);
    assert_eq!(limiter.try_acquire_at(12, 30), Ok(()));
    assert_eq!(limiter.capacity_remaining_or_0(15), 20);
    
    // Another window [20-29]
    assert_eq!(limiter.capacity_remaining_or_0(20), 50);
}

#[test]
fn test_rate_limiter_core_window_boundaries() {
    let limiter: Box<dyn RateLimitCore> = create_fixed_window_limiter(100, 5);
    
    // Test exact boundary transitions
    // Window 0: [0-4]
    assert_eq!(limiter.try_acquire_at(4, 60), Ok(())); // Last tick of window
    assert_eq!(limiter.capacity_remaining_or_0(4), 40);
    
    // Window 1: [5-9] - exactly at boundary
    assert_eq!(limiter.capacity_remaining_or_0(5), 100); // Reset at new window
    assert_eq!(limiter.try_acquire_at(5, 100), Ok(()));
    
    // Window 2: [10-14]
    assert_eq!(limiter.try_acquire_at(10, 50), Ok(()));
    assert_eq!(limiter.capacity_remaining_or_0(14), 50); // Last tick
    
    // Window 3: [15-19]
    assert_eq!(limiter.capacity_remaining_or_0(15), 100); // Reset again
}

#[test]
fn test_rate_limiter_core_skip_windows() {
    let limiter: Box<dyn RateLimitCore> = create_fixed_window_limiter(80, 10);
    
    // Use tokens in window 0 [0-9]
    assert_eq!(limiter.try_acquire_at(3, 40), Ok(()));
    
    // Jump multiple windows to tick 35 (window 3: [30-39])
    assert_eq!(limiter.capacity_remaining_or_0(35), 80); // Fresh window
    assert_eq!(limiter.try_acquire_at(37, 80), Ok(()));
    assert_eq!(limiter.capacity_remaining_or_0(38), 0);
}
...
fn test_rate_limiter_core_time_consistency() {
    ...
    let result: SimpleRateLimitResult = limiter.try_acquire_at(9, 10);
    ...
    assert_eq!(limiter.capacity_remaining_or_0(19), 50);
}

#[test]
fn test_rate_limiter_core_zero_operations() {
    let limiter: Box<dyn RateLimitCore> = create_fixed_window_limiter(100, 20);
...
fn test_rate_limiter_core_window_size_variations() {
    ...
    for (capacity, window_ticks) in configs {
        let limiter: Box<dyn RateLimitCore> = create_fixed_window_limiter(capacity, window_ticks);
        ...
        assert_eq!(limiter.capacity_remaining_or_0(0), capacity / 2);
        
        // Next window resets
        assert_eq!(limiter.capacity_remaining_or_0(window_ticks), capacity);
    }
}

#[test]
fn test_rate_limiter_core_interface_consistency() {
    let limiter: Box<dyn RateLimitCore> = create_fixed_window_limiter(75, 15);
    
    // Window 0 [0-14]
    assert_eq!(limiter.capacity_remaining_or_0(5), 75);
    ...
    assert_eq!(limiter.capacity_remaining_or_0(15), 75);
}

#[test]
fn test_rate_limiter_core_as_trait_object() {
    let limiter: Box<dyn RateLimitCore> = Box::new(FixedWindowCounterCore::new(50, 5));
    
    // Complex usage pattern
    // Window 0 [0-4]
    assert_eq!(limiter.try_acquire_at(2, 20), Ok(()));
    assert_eq!(limiter.try_acquire_at(4, 20), Ok(()));
    assert_eq!(limiter.capacity_remaining_or_0(4), 10);
...
fn test_rate_limiter_core_trait_bounds() {
    ...
    let _shared: Arc<dyn RateLimitCore> = Arc::new(FixedWindowCounterCore::new(100, 10));
}

#[test]
fn test_rate_limiter_core_window_alignment() {
    let limiter: Box<dyn RateLimitCore> = create_fixed_window_limiter(100, 7);
    ...
    assert_eq!(limiter.capacity_remaining_or_0(13), 50);
    
    // tick 20 -> window 2 (20/7=2), start_tick = 14
    assert_eq!(limiter.capacity_remaining_or_0(20), 100); // New window
    
    // tick 28 -> window 4 (28/7=4), start_tick = 28
    assert_eq!(limiter.try_acquire_at(28, 30), Ok(()));
    assert_eq!(limiter.capacity_remaining_or_0(34), 70); // Same window
}