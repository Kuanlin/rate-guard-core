...
fn test_bucket_index_calculation() {
    ...
    assert_eq!(counter.try_acquire_at(25, 20), Ok(())); // bucket 0 resets and adds 20
    
    // Current total: 0 + 10 + 10 + 20 = 40
    assert_eq!(counter.try_acquire_at(25, 60), Ok(())); // total = 100
...
fn test_sliding_window_range_calculation() {
    ...
    assert_eq!(counter.try_acquire_at(0, 20), Ok(()));
    
    // tick 10: bucket 2 [10-14], window [0, 10]
    // bucket 0 [0-4] start_tick=0 within window
    assert_eq!(counter.try_acquire_at(10, 20), Ok(())); // total = 20 + 20 = 40
...
fn test_same_tick_multiple_calls() {
    ...
    assert_eq!(counter.try_acquire_at(5, 15), Ok(()));
    ...
    assert_eq!(counter.try_acquire_at(5, 10), Ok(())); // total = 100
    ...
    assert_eq!(counter.try_acquire_at(25, 50), Ok(())); // bucket 1 resets and adds 50
    
    // Same tick again
    assert_eq!(counter.try_acquire_at(25, 50), Ok(())); // total = 100
    assert_eq!(counter.try_acquire_at(25, 1), Err(SimpleRateLimitError::InsufficientCapacity));
}
...
fn test_complex_bucket_expiry_scenarios() {
    ...
    assert_eq!(counter.try_acquire_at(2, 15), Ok(()));   // bucket 0 [0-4]
    assert_eq!(counter.try_acquire_at(7, 20), Ok(()));   // bucket 1 [5-9]  
    assert_eq!(counter.try_acquire_at(12, 25), Ok(()));  // bucket 2 [10-14]
    assert_eq!(counter.try_acquire_at(17, 20), Ok(()));  // bucket 3 [15-19]
    ...
    assert_eq!(counter.try_acquire_at(25, 15), Ok(())); // bucket 1 resets and adds 15
    // total = 15 + 25 + 20 = 60
    assert_eq!(counter.try_acquire_at(25, 20), Ok(())); // total = 80
    assert_eq!(counter.try_acquire_at(25, 1), Err(SimpleRateLimitError::InsufficientCapacity));
}
...
fn test_bucket_expiry_boundary_conditions() {
    ...
    assert_eq!(counter.try_acquire_at(1, 30), Ok(()));   // bucket 0
    assert_eq!(counter.try_acquire_at(4, 30), Ok(()));   // bucket 1
    assert_eq!(counter.try_acquire_at(7, 30), Ok(()));   // bucket 2
    
    // tick 9: bucket 0 new cycle [9-11], window [0, 9]
    // All buckets within window, but bucket 0 gets reset
    // total = 0 + 30 + 30 = 60
    assert_eq!(counter.try_acquire_at(9, 40), Ok(())); // bucket 0 resets and adds 40
    // total = 40 + 30 + 30 = 100
    ...
    // total = 40 + 0 + 30 = 70
    assert_eq!(counter.try_acquire_at(12, 30), Ok(())); // bucket 1 resets and adds 30
    // total = 40 + 30 + 30 = 100
    assert_eq!(counter.try_acquire_at(12, 1), Err(SimpleRateLimitError::InsufficientCapacity));
}
...
fn test_single_bucket_configuration() {
    ...
    assert_eq!(counter.try_acquire_at(5, 25), Ok(()));
    assert_eq!(counter.try_acquire_at(8, 25), Ok(())); // total = 50
    assert_eq!(counter.try_acquire_at(9, 1), Err(SimpleRateLimitError::InsufficientCapacity));
    
    // tick 15: bucket 0 new cycle [10-19], window [5, 15]
    // Old bucket 0 [0-9] start_tick=0 < 5, expires
    assert_eq!(counter.try_acquire_at(15, 50), Ok(())); // bucket 0 resets and adds 50
    
    // tick 25: bucket 0 new cycle [20-29], window [15, 25]  
    // Old bucket 0 [10-19] start_tick=10 < 15, expires
    assert_eq!(counter.try_acquire_at(25, 50), Ok(())); // bucket 0 resets and adds 50
}
...
fn test_high_frequency_buckets() {
    ...
    assert_eq!(counter.try_acquire_at(0, 10), Ok(()));  // bucket 0 [0]
    assert_eq!(counter.try_acquire_at(1, 10), Ok(()));  // bucket 1 [1]
    assert_eq!(counter.try_acquire_at(2, 10), Ok(()));  // bucket 2 [2]
    assert_eq!(counter.try_acquire_at(3, 10), Ok(()));  // bucket 3 [3]
    assert_eq!(counter.try_acquire_at(4, 10), Ok(()));  // bucket 4 [4]
    
    // tick 5: bucket 0 new cycle [5], window [0, 5]
    // All buckets within window, but bucket 0 gets reset
    // total = 0 + 10 + 10 + 10 + 10 = 40
    assert_eq!(counter.try_acquire_at(5, 10), Ok(())); // bucket 0 resets and adds 10
    // total = 10 + 10 + 10 + 10 + 10 = 50
    assert_eq!(counter.try_acquire_at(5, 10), Ok(())); // total = 60
    assert_eq!(counter.try_acquire_at(5, 1), Err(SimpleRateLimitError::InsufficientCapacity));
}
...
fn test_bucket_start_tick_precision() {
    ...
    assert_eq!(counter.try_acquire_at(2, 20), Ok(()));   // bucket 0, start_tick=0
    assert_eq!(counter.try_acquire_at(8, 25), Ok(()));   // bucket 1, start_tick=6
    assert_eq!(counter.try_acquire_at(14, 30), Ok(()));  // bucket 2, start_tick=12
    ...
    assert_eq!(counter.try_acquire_at(18, 25), Ok(())); // bucket 0 (new cycle), start_tick=18
    
    // Verify only valid buckets are counted
    assert_eq!(counter.try_acquire_at(18, 20), Ok(())); // Check total capacity
}