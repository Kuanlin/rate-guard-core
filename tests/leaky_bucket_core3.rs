...
fn test_rate_limiter_core_basic_acquire() {
    ...
    assert_eq!(limiter.try_acquire_at(20, 0), Ok(()));
...
fn test_rate_limiter_core_capacity_remaining() {
    ...
    // Add some tokens
    assert_eq!(limiter.try_acquire_at(40, 0), Ok(()));
...
fn test_rate_limiter_core_complete_drain() {
    ...
    // Can fill it again
    assert_eq!(limiter.try_acquire_at(30, 20), Ok(()));
}
...
fn test_rate_limiter_core_concurrent_behavior() {
    ...
    for i in 0..5 {
        ...
        let handle = thread::spawn(move || {
            let tick = i * 5;
            limiter_clone.try_acquire_at(10, tick)
        });
        handles.push(handle);
    }
...
fn test_rate_limiter_core_large_time_jump() {
    ...
    // Can fill again
    assert_eq!(limiter.try_acquire_at(100, 1000), Ok(()));
}
...
fn test_rate_limiter_core_as_trait_object() {
    ...
    // Fill to capacity
    assert_eq!(limiter.try_acquire_at(40, 25), Ok(()));
    assert_eq!(limiter.capacity_remaining(25), 75); // 35 + 40 = 75
    
    // At capacity
    assert_eq!(limiter.try_acquire_at(1, 25), Err(RateLimitError::ExceedsCapacity));
}
...
fn test_rate_limiter_core_polymorphic_usage() {
    ...
    for (i, limiter) in limiters.iter().enumerate() {
        ...
        let result = limiter.try_acquire_at(10, tick);
